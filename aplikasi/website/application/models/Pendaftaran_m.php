...
class Pendaftaran_m extends CI_Model {
	...
	public function get_all_pasien($status=null,$date=null,$id_pasien=null,$id_pendaftaran=null){
		$table = $this->table;
		$fields = "*, pendaftaran.status AS status_daftar, pas.alamat AS alamat_pasien, pendaftaran.id_pasien AS id_pasien, pendaftaran.id_pendaftaran as id_pendaftaran ";
		$join = " pasien AS pas ON pendaftaran.id_pasien = pas.id_pasien
				  ...
				  LEFT JOIN rekam_medis AS rk ON pendaftaran.id_pendaftaran = rk.id_pendaftaran";
		if($id_pasien){
			$where = " rsp.id_rs = '".$this->session->userdata('logged_in')['id_rs']."' AND pendaftaran.id_pasien = '".$id_pasien."' AND pendaftaran.status = '2' ";
		} else if($status == "-1"){
		...
		//get single data
		if($id_pendaftaran){
			$where.=" AND pendaftaran.id_pendaftaran = '".$id_pendaftaran."' ";
		}

		//get pasien untuk level akses dokter (pasien yg udah di approve pegawai)
		if($this->session->userdata('logged_in')['status'] == '2'){
			$where.=" AND d.id_dokter = '".$this->session->userdata('logged_in')['id_dokter']."' AND pendaftaran.status = 1 AND pendaftaran.status = 2 ";
		}
		
		// echo $where;
		$order = " pendaftaran.id_pendaftaran DESC ";
	...
	public function approve($id_pendaftaran, $status){
		$pasien = array();
